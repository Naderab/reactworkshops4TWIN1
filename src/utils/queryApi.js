  ...
  try {
    ...
    if (body) {
      ...
      if (["POST", "PUT", "PATCH"].includes(method.toUpperCase())) {
        if (transformBody) {
          // If our method is POST, PUT or PATCH, and we have to transform our bodyto Form Data (for files upload for example)
          ...
          // If not keep the content type json and the body will be parsedautomatically to json
          config = {
            ...config,
            headers: { "Content-Type": "application/json" },
            data: body,
    ...
    // This example uses localStorage, feel free to change it to cookie storage orsomething else.
